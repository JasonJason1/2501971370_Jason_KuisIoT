...
#define WIFI_SSID "Ribet"
#define WIFI_PASSWORD "asdf1234"

#define MQTT_BROKER  "broker.emqx.io"
#define MQTT_TOPIC_PUBLISH_TEMPERATURE "esp32_jason/temperature"
#define MQTT_TOPIC_PUBLISH_HUMIDITY "esp32_jason/humidity"
#define MQTT_TOPIC_PUBLISH_LIGHT "esp32_jason/light"  
...
void taskLightCheck(void *pvParameters) {
  while (true) {
    if(lux > 400) {
      Serial.printf("Warning: Pintu dibuka; Light: %.2f lux\n", lux);
    } else {
      Serial.printf("Pintu tertutup; Light: %.2f lux\n", lux);
    }
    vTaskDelay(4000 / portTICK_PERIOD_MS);
  }
}
